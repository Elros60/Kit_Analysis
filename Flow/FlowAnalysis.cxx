...
void FlowAnalysis(float dimuonMassMin = 0., float dimuonMassMax = 4.,
                  ...
                  bool fPropError = true, bool fCumulant = true) {
  ...
  float C22REF = 0.;  //! Event averaged 2-p cumulant for reference
  float C24REF = 0.;  //! Event averaged 4-p cumulant for reference
  float C22EREF = 0.; //! Error for event averaged 2-p cumulant for reference
  float C24EREF = 0.; //! Error for event averaged 4-p cumulant for reference
  ...
  if (fCumulant) {
    ...
    for (int i = 0; i < NBinsMult; i++) {
      ...
      if (Sum22REF[i] * Sum24REF[i] != 0.) {
        ...
        C24MultREF[i] =
            C24MultREF[i] / Sum24REF[i] - 2. * pow(C22MultREF[i], 2);
  ...
  float *V2SP = new float[NBinsMass];
  float *V2ESP = new float[NBinsMass];
  float *V2EP = new float[NBinsMass];
  float *V2EEP = new float[NBinsMass];
  float *V2MultBinMass = new float[NBinsMass];
  ...
  for (int i = 0; i < fChain_POI->GetEntries(); i++) {
    ...
    // Fill bins for SP and EP
    if (!(isnan(fR2SP) || isinf(fR2SP) || isnan(fR2EP) || isinf(fR2EP))) {
      for (int i = 0; i < NBinsMass; i++) {
        if (fMass > MassBins[i] && fMass <= MassBins[i + 1]) {
          V2SP[i] += fU2Q2 / fR2SP;
          V2ESP[i] += pow(fU2Q2 / fR2SP, 2);
          V2EP[i] += fCos2DeltaPhi / fR2EP;
          V2EEP[i] += pow(fCos2DeltaPhi / fR2EP, 2);
          V2MultBinMass[i] += 1.;
        }
      }
    }
  ...
  for (int i = 0; i < NBinsMass; i++) {
    ...
    if (V2MultBinMass[i] != 0.) {
      ...
      V2ESP[i] = pow(V2ESP[i] / V2MultBinMass[i] - V2SP[i] * V2SP[i], 1. / 2);

      V2EP[i] = V2EP[i] / V2MultBinMass[i];
      V2EEP[i] = pow(V2EEP[i] / V2MultBinMass[i] - V2EP[i] * V2EP[i], 1. / 2);
      if (fPropError) {
    ...
    if (fCumulant) {
      if (Sum22POI[i] * Sum24POI[i] != 0.) {
        ...
        V24EPOI[i] = pow(pow(-1. * C24REF, -6. / 4) * C24EPOI[i] * C24EPOI[i] +
                             pow(-1. * C24REF, -14. / 4) * C24POI[i] *
                                 C24POI[i] * C24EREF * C24EREF * 9. / 16,
                         1. / 2);
        if (fPropError) {
          if (isnan(C22EPOI[i])) {
